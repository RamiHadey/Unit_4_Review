...
//step 5 
function startUp(){
   //sets the text on Id puzzleTitle
...
function switchPuzzle(e){
   //adds an alert to tell you that you'll lose your work
   if(confirm("You will lose all of you work on the puzzle: continue?")){
      ...
      case "puzzle2":
         ...
         break;
      case "puzzle3":
         document.getElementById("puzzle").innerHTML = drawHitori(puzzle3Hint, puzzle3Rating, puzzle3);
      break;
   }
   setupPuzzle();
   }
...
function setupPuzzle(){
   ...
   for(var i = 0; i < allCells.length; i++){
      ...
      //when shift key is clicked it changes color and border
      ...
      //When certain keys are pressed the image of the cursor changes
      ...
      //step 7e when Check solutions button is pressed, the check solutions function runs
      allCells[i].addEventListener("mouseup", 
         checkSolution
      );
...
function findErrors(){
   for(var i = 0; i < allCells.length; i++){
      //checks the color and class name
      ...
      (allCells[i].className === "circles" && allCells[i].style.backgroundColor === ("black"))){
         //changes color to red font
         allCells[i].style.color = "red";
      }
   }

   //step 8b This checks if the color is red then changes it to white in one second
   setTimeout(
      function(){
         for(var i = 0; i < allCells.length; i++){
